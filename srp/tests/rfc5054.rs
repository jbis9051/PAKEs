...
fn rfc5054() {
    let i = b"alice";
    let p = b"password123";
    let s = hex::decode(strip_white("BEB25379 D1A8581E B5A72767 3A2441EE")).unwrap();
    let group = &G_1024;
    ...
    assert_eq!(
        k.to_bytes_be(),
        hex::decode(strip_white("7556AA04 5AEF2CDD 07ABAF0F 665C3E81 8913186F")).unwrap(),
        "bad k value"
    );
    ...
    assert_eq!(
        x.to_bytes_be(),
        hex::decode(strip_white("94B7555A ABE9127C C58CCF49 93DB6CF8 4D16C124")).unwrap(),
        "bad x value"
    );
    ...
    assert_eq!(
        u.to_bytes_be(),
        hex::decode(strip_white("CE38B959 3487DA98 554ED47D 70A7AE5F 462EF019")).unwrap(),
        "bad u value"
    );
